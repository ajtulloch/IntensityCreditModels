...
def CDSIssuance( barcolour = "grey", fig_width = 469.7549 ):
	...
	print CDS_Issuance.split()
	
	CDS 	= [ float(i)/1000 for n,i in enumerate(CDS_Issuance.split()) if n % 2 == 1]
	CDO 	= [ float(i)/1000 for n,i in enumerate(CDO_Issuance.split()) if n % 2 == 1]
	labels 	= [ i for n,i in enumerate(labels) if n % 2 == 1]
...
def LevyProcessPlots():
	...
	PRV = stats.poisson.rvs(0.1*dt, size = N)
	Poisson = ProcessGen(PRV)
		...
		for i, param in enumerate(parameter_names):
			...
			if acorr:
				if AUTOCOLOR:
					...
					pylab.legend((p, q), ("Dynamic", "Static"), loc = "center right")
				else:
...
def MertonStructuralModel():
	"""docstring for MertonStructuralModel"""
	barrier = 20
	...
	def Simulate(N):
		...
		for i in range(1, N):
			...
			# print "current S: ", S_C
			# print "Barrier hit?: ", barrier_hit
			# print "Barrier: ", barrier
			if barrier_hit:
				results[i] = barrier
			else:
				S_next = results[i-1] + 2*stats.norm.rvs() - 4.5 * dt
				if S_next < barrier:
	...
	barrier_array = [barrier for i in xs]
	pylab.clf()
	...
	pylab.figure(1)
	pylab.xlabel('Year' )
	pylab.ylabel('Asset Value ($V_t$)' )
	print "cats"
	if AUTOCOLOR:
		plot = pylab.plot(xs, results, label = "Asset Value")
		plot2 = pylab.plot(xs, barrier_array, label = "Barrier")
	else:
		plot = pylab.plot(xs, results, color = "black")
	# pylab.show()
	pylab.legend()
	pylab.ylim([0, 200])
	pylab.savefig('../../Diagrams/SimMerton.pdf')
